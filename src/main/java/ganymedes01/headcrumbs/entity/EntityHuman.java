...
public class EntityHuman extends EntityMob implements IRangedAttackMob, IHumanEntity {
    ...
    private static final AttributeModifier babySpeedBoostModifier = new AttributeModifier(
	    UUID.fromString("B9766B59-9566-4402-BC1F-2EE2A276D836"), "Baby speed boost", 0.5D, 1);
    ...
    // Data watcher
    private static final DataParameter<String> NAME = EntityDataManager.<String>createKey(EntityHuman.class,
	    DataSerializers.STRING);
    private static final DataParameter<Boolean> CHILD = EntityDataManager.<Boolean>createKey(EntityHuman.class,
	    DataSerializers.BOOLEAN);
    private static final DataParameter<Float> WIDTH = EntityDataManager.<Float>createKey(EntityHuman.class,
	    DataSerializers.FLOAT);
    private static final DataParameter<Float> HEIGHT = EntityDataManager.<Float>createKey(EntityHuman.class,
	    DataSerializers.FLOAT);
    private static final DataParameter<Boolean> DRAWING_BOW = EntityDataManager.<Boolean>createKey(EntityHuman.class,
	    DataSerializers.BOOLEAN);
	...
	targetTasks.addTask(2, new EntityAINearestAttackableTarget<EntityPlayer>(this, EntityPlayer.class, 0, true,
		false, new Predicate<EntityPlayer>() {
	...
	if (rand.nextFloat() < (worldObj.getDifficulty() == EnumDifficulty.HARD ? 0.1F : 0.05F)) {
		...
		if (worldObj.getDifficulty() == EnumDifficulty.HARD)
			...
			getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH)
				.applyModifier(new AttributeModifier("Shield Bonus", rand.nextDouble() * 3.0 + 1.0, 2));
		    }
	    } else if (i == 1) {
		...
		if (worldObj.getDifficulty() == EnumDifficulty.HARD)
			...
			setItemStackToSlot(EntityEquipmentSlot.OFFHAND,
				PotionUtils.addPotionToItemStack(new ItemStack(Items.TIPPED_ARROW), potion));
		    }
	    }
	}
    }
	...
	ItemMonsterPlacer.applyEntityIdToItemStack(stack, EntityList.func_191306_a(this.getClass()));
	return stack;
    }
	...
	getEntityAttribute(SharedMonsterAttributes.KNOCKBACK_RESISTANCE)
		.applyModifier(new AttributeModifier("Knockback Resistance Bonus", rand.nextDouble() * 0.05, 0));

	double rangeBonus = rand.nextDouble() * 1.5 * additionalDifficulty;
	if (rangeBonus > 1.0)
	    getEntityAttribute(SharedMonsterAttributes.FOLLOW_RANGE)
		    .applyModifier(new AttributeModifier("Range Bonus", rangeBonus, 2));

	if (rand.nextFloat() < additionalDifficulty * 0.05F)
	    getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH)
		    .applyModifier(new AttributeModifier("Health Bonus", rand.nextDouble() * 3.0 + 1.0, 2));

	if (rand.nextFloat() < additionalDifficulty * 0.15F)
	    getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE)
		    .applyModifier(new AttributeModifier("Damage Bonus", rand.nextDouble() + 0.5, 2));

	if (rand.nextFloat() < additionalDifficulty * 0.2F)
	    getEntityAttribute(SharedMonsterAttributes.MOVEMENT_SPEED)
		    .applyModifier(new AttributeModifier("Speed Bonus", rand.nextDouble() * 2.0 * 0.24 + 0.01, 2));
	...
	arrow.setDamage(
		damage * 2.0F + rand.nextGaussian() * 0.25 + worldObj.getDifficulty().getDifficultyId() * 0.11F);
	...
	if ("Herobrine".equals(name)) {
	    getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE)
		    .applyModifier(new AttributeModifier("Herobrine Damage Bonus", 1, 2));
	    getEntityAttribute(SharedMonsterAttributes.MOVEMENT_SPEED)
		    .applyModifier(new AttributeModifier("Herobrine Speed Bonus", 0.5, 2));
	}
