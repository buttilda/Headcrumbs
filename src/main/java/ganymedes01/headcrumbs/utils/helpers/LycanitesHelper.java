...
public class LycanitesHelper extends HeadDropHelper {
	...
	protected ItemStack getHeadForEntity(Entity entity) {
		/*
		 * String mobName = EntityList.getEntityString(entity); if (mobName == null) return null;
		 * 
		 * if (entity instanceof EntityPinky) return SkullTypes.pinky.getStack(); else if (entity instanceof EntityBehemoth) return SkullTypes.behemoth.getStack(); else if (entity instanceof EntityBelph) return SkullTypes.belph.getStack(); else { int dot = mobName.indexOf('.') + 1; if (dot > 1) { String prefix = mobName.substring(0, dot - 1); if (prefixes.contains(prefix)) { mobName = mobName.substring(dot).toLowerCase(); return !blacklist.contains(mobName) ? getStackFor(mobName) : null; } } }
		 */
		return null;
	}
